...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 23 15:21:59 2019
...
def predict_energy(input_data, context, emo):
    input_feats             = input_data[:,:128]
    input_energy            = input_data[:,129:]
    scaler                  = joblib.load('./data/'+emo+'-DNN/'+emo+'-energy-scaler-modif6.pkl')
    ...
    num_energy              = input_energy.shape[1]
    num_output              = 1
    
    X_feats                 = tf.placeholder("float", [None, num_input])
    X_energy                = tf.placeholder("float", [None, num_energy])
    ...
    weights = {
        ...
        'h3' : tf.Variable(tf.random_normal([n_hidden_2+num_energy, n_hidden_3])), 
    ...
    def neural_net(x_feats, x_energy, dropout, training):
        ...
        layer_2     = tf.concat([layer_2, x_energy], 1)
        ...
        layer_4     = tf.concat([layer_4, x_energy[:,context-1:context+2]], 1)
    
        out_layer   = tf.add(tf.matmul(layer_4, weights['out']), \
                                biases['out'])
        return tf.nn.relu(out_layer)
    
    regressor       = neural_net(X_feats, X_energy, keep_prob, training_mode)
    saver           = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, './data/'+emo+'-DNN/'+emo+'-energy-mle-modif6.ckpt')
        predictions = sess.run(regressor, feed_dict={X_feats:input_feats, \
                                                     X_energy:input_energy, \
                                                     keep_prob:1.0, \
                                                     training_mode:False})
    return predictions
